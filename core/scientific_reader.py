...
class ScientificRasterSpec:
    ...
    def expected_file_structure(self, file_size_bytes: Optional[int] = None) -> Dict[str, Any]:
        ...
        line_prefix = int(self.line_prefix_bytes or 0)
        line_suffix = int(self.line_suffix_bytes or 0)
        record_bytes = int(self.record_bytes) if self.record_bytes else (
            line_prefix + self.samples * bytes_per_sample + line_suffix
        )
        expected_raster = int(self.lines) * record_bytes
    ...
    def read_preview(self, file_or_bytes: Union[bytes, Any], name: str, max_side: int = 2048) -> Tuple[np.ndarray, np.ndarray, Dict[str, Any]]:
        raise NotImplementedError("Use load_scientific_preview instead")
