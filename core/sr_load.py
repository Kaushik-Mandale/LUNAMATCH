...
def load_scientific_preview(file_or_bytes: Any, name: str, metadata: Optional[Dict[str, Any]] = None, max_side: int = 1024):
    metadata = apply_lroc_nac_edr_pds3_profile(metadata or {})
    spec = scientific_raster_spec(metadata)
    ...
    try:
        ...
        # Hard-cap preview so Cloud RSS stays bounded even if caller passes 4096
        effective_side = min(int(max_side), 1536)
        step_y = max(1, int(math.ceil(shape[0] / effective_side)))
        step_x = max(1, int(math.ceil(shape[1] / effective_side)))
        preview = np.asarray(mm[::step_y, ::step_x], dtype=np.float32)
        del mm
        gc.collect()
...
def load_product(file_or_bytes, name: str, metadata=None, max_side: int = 1024):
    ptype = classify_product(name)
